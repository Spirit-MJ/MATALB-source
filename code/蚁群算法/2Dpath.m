...
position = [40,120;40,170;80,170;80,120;
            ...
            80,100;80,110;120,110;120,100];  % 障碍物
...
rou = 0.2;
view_h = 10;  % 横向可视域大小
view_w = abs(End(2) - Start(2)) + 1;  % 纵向可视域大小 
...
% 初始搜索路径
[path,T] = find_path(ant_num, num_gen, Start, End, view_h, T, T_fill);  % 初始搜索路径
...
loc_x = [loc_x,End(1)];
figure(1)
plot(loc_x,zuiyou_route(end,:),'r-.'); % 规划空间
...
for ii = 1:ant_num
    ...
    for loc_x = 1:num_gen  % 横向可视域
        ...
        for i = 90:180  % 纵向搜索
            ...
            qfz(kk) = comput_qfz(Start(1)+(loc_x-1)*view_h, now_point, next_point(kk),End(1),End(2),view_h);  % 计算启发值
            qz(kk) = qfz(kk) * T(loc_x,next_point(kk)-90+1) * T_fill(loc_x,next_point(kk)-90+1);  % 计算可视域中点的概率
            kk = kk+1;
        end
        %轮盘赌选择访问的下一个地点
        p = qz/sum(qz);
        ...
        %更新信息素
        T(loc_x,old_point-90+1) = 0.9 * T(loc_x,old_point-90+1);  % 信息素挥发
