...
ng=300;%进化代数
ggap=0.95;%种群代沟
variatenum=core*2;%变量个数
ca=variatenum/2;%类别数
len=10;%编码长度
yuanshi(x,y,ca);%画出不经优化的聚类图
chrom=round(rand(np,variatenum*len));%初始化种群
Field=[repmat(len,1,variatenum);repmat([min(x) min(y);max(x) max(y)],1,variatenum/2);repmat([1;0;1;1],1,variatenum)];%区域描述器
...
for i=1:variatenum/2
      for j=1:num(i)
          J=J+sqrt((Kx(i,j)-x0(i))^2+(Ky(i,j)-y0(i))^2);
      end
end
fitx=1/(1+J);
end
